...
namespace GAMO.Common {
    ...
    */
    public class JsonObject {
        ...
        // linh.nguyenthaitung - convenience function to create array object (im so lazy, i know)
        public static JsonObject Create(string[] strArray) {
            JsonObject obj = arr;
            foreach (string item in strArray)
                obj.list.Add(CreateStringObject(item));
            return obj;
        }
        // end
            ...
            if (!string.IsNullOrEmpty(str)) {
                ...
                if (str.Length > 0) {
                    if (str.ToUpper().Equals("TRUE")) {
                        ...
                        type = Type.BOOL;
                        b = false;
                    } else if (str.ToUpper().Equals("NULL")) {
                        type = Type.NULL;
#if USEFLOAT
